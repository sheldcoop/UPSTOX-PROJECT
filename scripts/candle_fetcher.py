...
def fetch_candles(
    ...
    url = f"{API_BASE}/historical-candle/{symbol}/{api_timeframe}/{end_date}/{start_date}"
...
def parse_candle(candle: List, symbol: str, instrument_key: str, timeframe: str) -> Optional[Dict]:
    """
...
def fetch_and_store(
    ...
    for display_name, instrument_key in symbol_list:
        ...
        try:
            ...
            # Parse candles
            parsed = [parse_candle(c, display_name, instrument_key, timeframe) for c in raw_candles]
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Fetch historical candles from Upstox API')
    parser.add_argument('--symbols', type=str, help='Comma-separated symbols (e.g., "INFY,TCS,RELIANCE")')
    parser.add_argument('--timeframe', type=str, default='1d', help='1m, 5m, 15m, 1h, 1d (default: 1d)')
    parser.add_argument('--start', type=str, help='Start date YYYY-MM-DD (default: 1 year ago)')
    parser.add_argument('--end', type=str, help='End date YYYY-MM-DD (default: today)')
    parser.add_argument('--db-query', type=str, help='SQL WHERE clause (e.g., "segment=\'NSE_EQ\' AND has_fno=1")')
    parser.add_argument('--delay', type=float, default=0.5, help='Rate limit delay between API calls (default: 0.5s)')
