...
class CorporateAnnouncementsFetcher:
    ...
    def __init__(self, access_token: Optional[str] = None, db_path: str = "market_data.db"):
        """
        ...
        if not self.access_token:
            logger.warning("No access token provided. Will use NSE scraping for corporate actions.")
    ...
    def _init_database(self):
        ...
        # Create indexes
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_announcements_symbol ON corporate_announcements(symbol)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_announcements_date ON corporate_announcements(event_date)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_earnings_symbol ON earnings_calendar(symbol)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_earnings_date ON earnings_calendar(earnings_date)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_alerts_symbol ON announcement_alerts(symbol)")
            ...
            if data.get('status') == 'success':
                ...
                for action in actions:
                    self._store_announcement({
                        ...
                        'amount': action.get('dividend_amount') or action.get('bonus_ratio'),
    ...
    def _fetch_from_nse(self, symbol: str, from_date: Optional[str] = None, 
        ...
        try:
            ...
            logger.info(f"Fetching NSE announcements for {symbol} from {from_date} to {to_date}...")
            ...
            for item in data:
                ...
                announcement = {
                    ...
                    'announcement_type': self._classify_nse_announcement(item.get('desc', '')),
                    ...
                    'impact_level': self._calculate_impact_from_description(item.get('desc', '')),
            ...
            logger.info(f"Successfully fetched {len(announcements)} announcements from NSE")
    ...
    def _classify_nse_announcement(self, description: str) -> str:
        ...
        if any(word in desc_lower for word in ['dividend', 'interim dividend', 'final dividend']):
            return 'DIVIDEND'
        elif any(word in desc_lower for word in ['split', 'stock split', 'sub-division']):
            return 'STOCK_SPLIT'
        ...
        elif any(word in desc_lower for word in ['result', 'quarterly', 'annual', 'financial']):
            return 'EARNINGS'
        elif any(word in desc_lower for word in ['agm', 'annual general meeting']):
            return 'AGM'
        elif any(word in desc_lower for word in ['egm', 'extraordinary general meeting']):
            return 'EGM'
        elif any(word in desc_lower for word in ['merger', 'acquisition', 'amalgamation']):
            return 'M&A'
        else:
            return 'OTHER'
    ...
    def _calculate_impact_from_description(self, description: str) -> str:
        ...
        if any(word in desc_lower for word in [
            'result', 'quarterly', 'annual', 'earnings',
    ...
    def get_earnings_calendar(self, symbol: Optional[str] = None, days_ahead: int = 90) -> List[Dict[str, Any]]:
        """
    ...
    def _generate_mock_earnings(self, symbol: str, days_ahead: int = 90) -> List[Dict[str, Any]]:
    ...
    def get_dividend_announcements(self, symbol: Optional[str] = None, days_ahead: int = 90) -> List[Dict[str, Any]]:
        """
    ...
    def _generate_mock_dividends(self, symbol: str, days_ahead: int = 90) -> List[Dict[str, Any]]:
    ...
    def get_upcoming_events(self, days_ahead: int = 30, impact_level: Optional[str] = None) -> List[Dict[str, Any]]:
        """
    ...
    def set_alert(self, symbol: str, announcement_type: str, days_before: int = 7) -> bool:
        """
    ...
    def _calculate_impact_level(self, action: Dict[str, Any]) -> str:
        ...
        high_impact = ['EARNINGS', 'STOCK_SPLIT', 'BONUS', 'MERGER', 'ACQUISITION', 'RIGHTS_ISSUE']
        medium_impact = ['DIVIDEND', 'INTERIM_DIVIDEND', 'FINAL_DIVIDEND', 'BOARD_MEETING']
    ...
    def display_upcoming_events(self, events: List[Dict[str, Any]]):
        ...
        print("=" * 120)
        print(f"{'Symbol':<12} | {'Event Type':<20} | {'Event Date':<12} | {'Impact':<8} | {'Days Away':<10} | {'Description':<30}")
        print("-" * 120)
        ...
        for event in events:
            ...
            print(f"{symbol:<12} | {event_type:<20} | {event_date:<12} | {impact_emoji} {impact:<6} | {days_str:<10} | {description:<30}")
    ...
    def display_earnings_calendar(self, earnings: List[Dict[str, Any]]):
        ...
        print("=" * 100)
        print(f"{'Symbol':<12} | {'Quarter':<8} | {'Earnings Date':<14} | {'Time':<6} | {'Days Away':<10} | {'Est. EPS':<10}")
        print("-" * 100)
        ...
        for earning in earnings:
            ...
            print(f"{symbol:<12} | {quarter:<8} | {earnings_date:<14} | {time:<6} | {days_str:<10} | {eps_str:<10}")
        
        print("=" * 100)
        print(f"Total earnings: {len(earnings)}\n")
    
    def monitor_announcements(self, interval: int = 3600, duration: Optional[int] = None):
        """
...
def main():
    ...
    parser.add_argument('--action', type=str, required=True,
                       choices=['earnings', 'dividends', 'upcoming', 'high-impact', 
                               'corporate-actions', 'set-alert', 'check-alerts', 'monitor'],
                       help='Action to perform')
    
    parser.add_argument('--symbol', type=str,
                       help='Trading symbol (e.g., NSE_EQ|INE009A01021 or INFY)')
    ...
    parser.add_argument('--announcement-type', type=str,
                       help='Type of announcement for alert')
    ...
    if args.action == 'earnings':
        symbols = args.symbols.split(',') if args.symbols else [args.symbol] if args.symbol else [None]
        
        for symbol in symbols:
            earnings = fetcher.get_earnings_calendar(symbol=symbol, days_ahead=args.days)
        ...
        for symbol in symbols:
            dividends = fetcher.get_dividend_announcements(symbol=symbol, days_ahead=args.days)
        ...
        if success:
            print(f"✅ Alert set for {args.symbol} {args.announcement_type} ({args.days_before} days before)")
        else:
