...
DB_PATH = __import__('os').path.join(__import__('os').path.dirname(__file__), '..', 'market_data.db')
...
def populate(db_path, instruments):
    ...
    try:
        ...
        for item in instruments:
            ...
            trading_symbol = item.get('trading_symbol') or item.get('short_name') or item.get('name')
            ...
            cur.execute(
                '''INSERT OR IGNORE INTO exchange_listings(instrument_key, symbol, trading_symbol, exchange, segment, exchange_token, lot_size, tick_size, instrument_type, underlying_symbol, underlying_type, last_updated)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, strftime('%s','now'))''',
                (instrument_key, trading_symbol, trading_symbol, exchange, segment, exchange_token, lot_size, tick_size, instrument_type, underlying_symbol, underlying_type)
            )
            if cur.rowcount:
                inserted_listing += 1
