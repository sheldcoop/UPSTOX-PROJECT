...
def get_historical_data(instrument_key, interval="day", days_back=30):
    ...
    try:
        ...
        logger.info(
            f"   Interval: {interval}, Period: {from_date_str} to {to_date_str}"
        )
        ...
        if data.get("status") == "success":
            ...
            if candles:
                ...
                for candle in candles[:10]:
                    ...
                    print(
                        f"{timestamp_str:<20} {open_price:>10.2f} {high_price:>10.2f} {low_price:>10.2f} {close_price:>10.2f} {volume:>12,}"
                    )
                ...
                if close_prices:
                    ...
                    print(
                        f"   Average Close: ₹{sum(close_prices)/len(close_prices):,.2f}"
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--symbol", "-s", default="TCS", help="Stock symbol (e.g., TCS, RELIANCE)"
    )
    parser.add_argument(
        "--interval",
        "-i",
    ...
    parser.add_argument(
        "--days", "-d", type=int, default=30, help="Number of days to fetch"
    )
    parser.add_argument(
        "--instrument-key", "-k", help="Direct instrument key (overrides symbol)"
    )
