...
class MarketQuoteV3:
    ...
    def get_batch_quotes(
        ...
        if use_cache:
            ...
            logger.info(
                f"📊 Cache: {cache_hits} hits, {len(uncached_keys)} misses"
            )
        else:
        ...
        self._save_metrics(
            "batch", len(unique_keys), cache_hits, api_calls, elapsed_ms
        )
            ...
            if self.use_v3:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.info(
                            f"✅ Fetched {len(quotes_data)} quotes (v3)"
                        )
                        return quotes_data
        ...
        try:
            with self.db_pool.get_connection() as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?)
                """, (request_type, instruments_count, cache_hits, api_calls, total_time_ms))
