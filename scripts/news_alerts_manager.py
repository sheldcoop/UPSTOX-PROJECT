...
class NewsAlertsManager:
    ...
    def __init__(self, db_path: str = "market_data.db", news_api_key: Optional[str] = None):
        """
    ...
    def _init_database(self):
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_news_symbols ON news_articles(symbols)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_news_sentiment ON news_articles(sentiment)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_alerts_symbol ON news_alerts(symbol)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_sentiment_symbol ON sentiment_history(symbol)")
    ...
    def _store_news(self, news: Dict[str, Any]):
        ...
        news_id = self._generate_news_id(
            news.get('headline', ''),
            news.get('published_at', str(datetime.now()))
        )
    ...
    def _update_sentiment_history(self, symbol: str, sentiment: str):
        ...
        if result:
            ...
            pos, neg, neu = (1, 0, 0) if sentiment == 'POSITIVE' else \
                            (0, 1, 0) if sentiment == 'NEGATIVE' else (0, 0, 1)
            total = 1
            avg_score = 1.0 if sentiment == 'POSITIVE' else -1.0 if sentiment == 'NEGATIVE' else 0.0
    ...
    def fetch_latest_news(self, symbol: Optional[str] = None, limit: int = 10, use_mock: bool = False) -> List[Dict[str, Any]]:
        """
        ...
        if self.news_api_key and not use_mock:
            try:
                ...
                logger.info(f"Fetched {len(news_articles)} real news articles from NewsAPI.org")
            except Exception as e:
                logger.warning(f"NewsAPI.org failed: {e}. Using mock data.")
                news_articles = self._generate_mock_news(symbol, limit)
        else:
            if not self.news_api_key:
                logger.warning("No NEWS_API_KEY found. Using mock data. Get free key: https://newsapi.org/register")
            news_articles = self._generate_mock_news(symbol, limit)
    ...
    def _generate_mock_news(self, symbol: Optional[str], count: int = 10) -> List[Dict[str, Any]]:
        ...
        symbols_list = [symbol] if symbol else ['INFY', 'TCS', 'RELIANCE', 'HDFC', 'ICICI']
        ...
        for i in range(count):
            ...
            news = {
                ...
                'author': 'Market Desk',
                'published_at': (current_time - timedelta(minutes=i*15)).strftime('%Y-%m-%d %H:%M:%S'),
                'category': 'EARNINGS' if 'earnings' in template['headline'] else 'GENERAL',
                'priority': 'HIGH' if template['sentiment_bias'] == 'NEGATIVE' else 'MEDIUM',
    ...
    def _fetch_from_newsapi(self, symbol: Optional[str], limit: int = 10) -> List[Dict[str, Any]]:
        """
    ...
    def _categorize_news(self, title: str) -> str:
        ...
        if any(word in title_lower for word in ['earnings', 'quarterly', 'results', 'profit', 'revenue']):
            return 'EARNINGS'
        elif any(word in title_lower for word in ['dividend', 'payout']):
            return 'DIVIDEND'
        elif any(word in title_lower for word in ['merger', 'acquisition', 'deal']):
            return 'M&A'
        elif any(word in title_lower for word in ['regulation', 'sebi', 'rbi', 'government']):
            return 'REGULATORY'
    ...
    def get_news_from_db(self, symbol: Optional[str] = None, 
    ...
    def search_news(self, keyword: str, days: int = 30, limit: int = 50) -> List[Dict[str, Any]]:
        """
        Search news by keyword.
    ...
    def get_sentiment_analysis(self, symbol: str, days: int = 30) -> Dict[str, Any]:
        ...
        if overall and overall[3]:
            total_positive, total_negative, total_neutral, total_articles, overall_sentiment = overall
            ...
            return {
                ...
                'positive_pct': (total_positive / total_articles * 100) if total_articles else 0,
                'negative_pct': (total_negative / total_articles * 100) if total_articles else 0,
                'neutral_pct': (total_neutral / total_articles * 100) if total_articles else 0,
                'overall_sentiment': overall_sentiment,
                'sentiment_rating': 'BULLISH' if overall_sentiment > 0.2 else 'BEARISH' if overall_sentiment < -0.2 else 'NEUTRAL',
    ...
    def add_to_watchlist(self, symbol: str, keywords: Optional[List[str]] = None, 
    ...
    def monitor_news(self, symbols: Optional[List[str]] = None, 
                    interval: int = 300, duration: Optional[int] = None):
        """
        Monitor news in real-time.
            ...
            while True:
                ...
                for symbol in (symbols or [None]):
                    ...
                    new_articles = [
                        ...
                        datetime.strptime(n['published_at'], '%Y-%m-%d %H:%M:%S') > last_check
                    ]
                    ...
                    if new_articles:
                        ...
                        for article in new_articles:
                            ...
                            print(f"{sentiment_emoji} [{article.get('published_at')}] {article.get('headline')}")
                            print(f"   {article.get('summary', '')[:100]}...")
                        print()
                
                last_check = datetime.now()
                print(f"✓ Checked at {last_check.strftime('%H:%M:%S')} - Next check in {interval}s")
    ...
    def display_news(self, news: List[Dict[str, Any]], title: str = "NEWS ARTICLES"):
        ...
        for i, article in enumerate(news, 1):
            ...
            print(f"   Published: {article.get('published_at')} | Source: {article.get('source')}")
    ...
    def display_sentiment(self, sentiment_data: Dict[str, Any]):
        ...
        print(f"   🟢 Positive: {sentiment_data['positive_count']} ({sentiment_data['positive_pct']:.1f}%)")
        print(f"   🔴 Negative: {sentiment_data['negative_count']} ({sentiment_data['negative_pct']:.1f}%)")
        print(f"   ⚪ Neutral:  {sentiment_data['neutral_count']} ({sentiment_data['neutral_pct']:.1f}%)")
        print()
...
def main():
    ...
    parser.add_argument('--action', type=str, required=True,
                       choices=['latest', 'breaking', 'search', 'sentiment',
                               'monitor', 'add-watchlist'],
    ...
    parser.add_argument('--limit', type=int, default=10,
                       help='Number of articles (default: 10)')
    ...
    if args.action == 'latest':
        ...
        manager.display_news(news, title=f"LATEST NEWS - {args.symbol if args.symbol else 'ALL'}")
        ...
        news = manager.search_news(keyword=args.keyword, days=args.days, limit=args.limit)
        ...
        success = manager.add_to_watchlist(
            symbol=args.symbol,
