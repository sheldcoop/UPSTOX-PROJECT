...
class RiskManager:
    ...
    def set_stop_loss(self, symbol: str, entry_price: float, 
                     stop_loss_price: float, quantity: int,
    ...
    def check_daily_loss(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'loss_percentage': (abs(daily_pnl) / self.max_daily_loss * 100) if daily_pnl < 0 else 0,
    ...
    def get_position_limits(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'utilization_pct': (total_position_value / self.max_position_size * 100) 
                               if self.max_position_size > 0 else 0
        }
...
def main():
    ...
    if args.action == 'size':
        if not all([args.symbol, args.entry, args.stop_loss]):
            print("Error: --symbol, --entry, and --stop-loss required for position sizing")
            return
        ...
        print("\n=== Position Limits ===")
        print(f"Active Positions: {limits['active_positions']} / {limits['max_positions']}")
