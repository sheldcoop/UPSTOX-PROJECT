...
def get_ltp_v3():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.warning(
                f"[TraceID: {g.trace_id}] LTP v3 API returned {response.status_code}"
            )
            return (
...
def get_ohlc_v3():
    ...
    try:
        ...
        logger.info(
            f"[TraceID: {g.trace_id}] OHLC v3 request - symbol: {symbol}, interval: {interval}"
        )
        ...
        if response.status_code == 200:
            ...
            logger.warning(
                f"[TraceID: {g.trace_id}] OHLC v3 API returned {response.status_code}"
            )
            return (
                jsonify({"error": f"API error: {response.status_code}"}),
                response.status_code,
...
def get_option_greeks_v3():
    ...
    try:
        ...
        logger.info(
            f"[TraceID: {g.trace_id}] Option Greeks v3 request - symbol: {symbol}"
        )
        ...
        if response.status_code == 200:
            ...
            logger.warning(
                f"[TraceID: {g.trace_id}] Option Greeks v3 API returned {response.status_code}"
            )
            return (
                jsonify({"error": f"API error: {response.status_code}"}),
                response.status_code,
        ...
        logger.error(
            f"[TraceID: {g.trace_id}] Option Greeks v3 failed: {e}", exc_info=True
        )
        return jsonify({"error": str(e), "trace_id": g.trace_id}), 500
