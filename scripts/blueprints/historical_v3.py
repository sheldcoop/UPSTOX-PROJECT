...
@historical_v3_bp.route("/historical-candle/<path:instrument_key>/<interval>/<count>/<to_date>/<from_date>", methods=["GET"])
@historical_v3_bp.route("/historical-candle/<path:instrument_key>/<interval>/<count>/<to_date>", methods=["GET"])
def get_historical_candle_v3(instrument_key, interval, count, to_date, from_date=None):
    ...
    try:
        ...
        logger.info(f"[TraceID: {g.trace_id}] Historical v3 request - {instrument_key}, {interval}, count: {count}")
        ...
        if response.status_code == 200:
            ...
            logger.warning(f"[TraceID: {g.trace_id}] Historical v3 API returned {response.status_code}")
            return jsonify({"error": f"API error: {response.status_code}"}), response.status_code
            
    except Exception as e:
        logger.error(f"[TraceID: {g.trace_id}] Historical v3 failed: {e}", exc_info=True)
        return jsonify({"error": str(e), "trace_id": g.trace_id}), 500


@historical_v3_bp.route("/historical-candle/intraday/<path:instrument_key>/<interval>/<count>", methods=["GET"])
def get_intraday_candle_v3(instrument_key, interval, count):
    ...
    try:
        ...
        logger.info(f"[TraceID: {g.trace_id}] Intraday v3 request - {instrument_key}, {interval}, count: {count}")
        ...
        if response.status_code == 200:
            ...
            logger.warning(f"[TraceID: {g.trace_id}] Intraday v3 API returned {response.status_code}")
            return jsonify({"error": f"API error: {response.status_code}"}), response.status_code
