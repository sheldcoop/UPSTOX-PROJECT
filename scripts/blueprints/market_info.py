...
def get_market_status(exchange=None):
    ...
    try:
        ...
        logger.info(f"[TraceID: {g.trace_id}] Market status request - exchange: {exchange}, segment: {segment}")
        ...
        logger.error(f"[TraceID: {g.trace_id}] Market status failed: {e}", exc_info=True)
...
def get_market_holidays():
    ...
    try:
        ...
        logger.info(f"[TraceID: {g.trace_id}] Holidays request - year: {year}, exchange: {exchange}")
        ...
        logger.error(f"[TraceID: {g.trace_id}] Holidays request failed: {e}", exc_info=True)
        return jsonify({"error": str(e), "trace_id": g.trace_id}), 500
...
def get_holiday_for_date(date_str):
    ...
    try:
        ...
        logger.error(f"[TraceID: {g.trace_id}] Holiday check failed: {e}", exc_info=True)
        return jsonify({"error": str(e), "trace_id": g.trace_id}), 500
...
def get_market_timings(date_str=None):
    ...
    try:
        ...
        logger.info(f"[TraceID: {g.trace_id}] Market timings request - date: {date_str}, exchange: {exchange}, segment: {segment}")
        ...
        logger.error(f"[TraceID: {g.trace_id}] Timings request failed: {e}", exc_info=True)
        return jsonify({"error": str(e), "trace_id": g.trace_id}), 500
