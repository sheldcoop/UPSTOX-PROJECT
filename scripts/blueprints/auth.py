    ...
    try:
        ...
        try:
            ...
            if response.status_code in [200, 204]:
                logger.info(f"[TraceID: {g.trace_id}] Token revoked successfully with Upstox")
            else:
                logger.warning(f"[TraceID: {g.trace_id}] Upstox logout returned {response.status_code}")
        except Exception as api_error:
            logger.warning(f"[TraceID: {g.trace_id}] Failed to revoke token with Upstox: {api_error}")
        ...
        logger.info(f"[TraceID: {g.trace_id}] Logout complete - {rows_affected} tokens deactivated")
...
def auth_status():
    ...
    try:
        ...
        logger.error(f"[TraceID: {g.trace_id}] Auth status check failed: {e}", exc_info=True)
        return jsonify({"error": str(e), "trace_id": g.trace_id}), 500
