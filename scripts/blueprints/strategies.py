...
def create_calendar_spread_strategy():
    ...
    try:
        ...
        logger.error(
            f"[TraceID: {g.trace_id}] Calendar spread error: {e}", exc_info=True
        )
...
def create_diagonal_spread_strategy():
    ...
    try:
        ...
        logger.error(
            f"[TraceID: {g.trace_id}] Diagonal spread error: {e}", exc_info=True
        )
        return jsonify({"error": str(e)}), 500
...
def create_double_calendar_strategy():
    ...
    try:
        ...
        logger.error(
            f"[TraceID: {g.trace_id}] Double calendar error: {e}", exc_info=True
        )
        return jsonify({"error": str(e)}), 500
