    ...
    def get_profile(self) -> Optional[Dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                logger.info(f"Profile fetched for user: {data.get('data', {}).get('user_name', 'Unknown')}")
                return data.get('data')
            else:
                logger.error(f"Profile failed: {response.status_code} - {response.text}")
                return None
    ...
    def get_funds(self) -> Optional[Dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                funds = data.get('data')
                if funds:
                    logger.info(f"Funds fetched - Available: ₹{funds.get('available_margin', 0):,.2f}")
                return funds
            else:
...
if __name__ == "__main__":
    ...
    if holdings:
        for h in holdings[:3]:  # Show first 3
            print(f"   - {h.get('tradingsymbol', 'Unknown')}: {h.get('quantity', 0)} shares")