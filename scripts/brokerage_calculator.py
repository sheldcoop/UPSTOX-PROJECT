...
class BrokerageCalculator:
    ...
    def calculate_charges(self, instrument_token: str, quantity: int, 
                         price: float, transaction_type: str, 
                         product: str = "D") -> Optional[Dict]:
        """
        ...
        try:
            ...
            if data.get("status") == "success":
                ...
                total_cost = (price * quantity) + total_charges if transaction_type.upper() == "BUY" else (price * quantity) - total_charges
    ...
    def save_calculation(self, symbol: str, quantity: int, price: float,
                        side: str, product: str, charges: Dict):
        ...
        c.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            symbol, quantity, price, side, product,
    ...
    def compare_products(self, symbol: str, quantity: int, price: float,
                        side: str) -> Dict:
    ...
    def print_charges_breakdown(self, charges: Dict, symbol: str, 
...
def main():
    ...
    parser.add_argument('--side', type=str, choices=['BUY', 'SELL'], 
                       help='Order side')
    parser.add_argument('--product', type=str, choices=['CNC', 'MIS', 'NRML'],
                       default='CNC', help='Product type')
    parser.add_argument('--compare', action='store_true',
                       help='Compare charges across all products')
    parser.add_argument('--action', type=str, choices=['history'],
                       help='Get calculation history')
    ...
    if args.action == 'history':
        ...
        for item in history:
            print(f"{item['timestamp']} | {item['symbol']:8} | "
