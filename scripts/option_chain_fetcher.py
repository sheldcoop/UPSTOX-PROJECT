...
def fetch_option_chain(
    underlying_key: str, expiry_date: str, access_token: str
) -> Optional[List[Dict]]:
    """
...
def parse_option_data(
    strike_data: Dict, underlying_key: str, expiry_date: str, snapshot_ts: int
...
def store_option_chain_snapshot(
    underlying_key: str, expiry_date: str, snapshot_ts: int, raw_json: str
) -> bool:
...
def fetch_and_store_option_chain(
    underlying_name: str, expiry_date: Optional[str] = None, delay: float = 1.0
) -> bool:
    """
    ...
    if expiry_date:
        if expiry_date not in expiries:
            logger.error(
                f"✗ Expiry '{expiry_date}' not available. Available: {expiries}"
            )
            return False
