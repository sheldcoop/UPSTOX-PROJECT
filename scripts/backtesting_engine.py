...
class OptionLeg:
    ...
    def __init__(self, option_type: str, action: str, strike: float, premium: float, qty: int = 1):
...
class Backtester:
    ...
    def run_backtest(
        ...
        try:
            ...
            pnl_percent = (pnl / abs(strategy.entry_cost)) * 100 if strategy.entry_cost != 0 else 0
            ...
            logger.info(f"Backtest complete: {strategy.name} | P&L: ₹{pnl:.2f} ({pnl_percent:.2f}%)")
    ...
    def run_multiple_backtests(
        ...
        if results:
            ...
            logger.info(f"Backtest summary: {total_trades} trades | Win rate: {win_rate:.1f}% | Total P&L: ₹{total_pnl:.2f}")
