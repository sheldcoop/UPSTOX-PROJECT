...
def normalize_market_cap(market_cap_value):
    ...
    if 'cr' in cap_str.lower():
        numeric_part = float(cap_str.lower().replace('cr', '').replace('crore', '').strip())
        return numeric_part * 10000000  # 1 crore = 10 million
    
    # Check for lakh/lac suffix
    if 'la' in cap_str.lower():
        numeric_part = float(cap_str.lower().replace('lakh', '').replace('lac', '').strip())
...
def standardize_nse_data(input_csv, index_code, index_type, output_csv=None):
    ...
    standardized_df['index_name'] = df.get('index_name', index_code.replace('_', ' ').title())
    ...
    if 'free_float_factor' in df:
        standardized_df['free_float_factor'] = df['free_float_factor'].apply(normalize_weight)
    else:
    ...
    standardized_df['raw_row_json'] = df.apply(lambda x: json.dumps(x.to_dict()), axis=1)
    ...
    print(f"  Missing ISIN: {missing_isin.sum()} ({missing_isin.sum()/len(standardized_df)*100:.2f}%)")
    ...
    print(f"\n✓ Standardization complete!")
    print(f"✓ Ready for import using: ./sample_import_script.sh {output_csv} {index_code}")
...
def main():
    """Main entry point."""
    if len(sys.argv) < 4:
        print("Usage: python standardize_nse_data.py <input_csv> <index_code> <index_type>")
