...
def render_page(state):
    ...
    Components.section_header(
        "Live Positions", "Manage and monitor your open trades", "pie_chart"
    )
            ...
            if "error" in positions_data:
                ...
                with Components.card():
                    with ui.row().classes("items-center gap-4"):
                        ...
                        with ui.column():
                            ui.label("Failed to Load Positions").classes(
                                "text-xl font-bold text-red-400"
                            )
                            ui.label(f"Error: {positions_data['error']}").classes(
                                "text-slate-400"
                            )
                ...
                with Components.card().classes("items-center justify-center py-12"):
                    ...
                    ui.label("No Active Positions").classes(
                        "text-2xl font-bold text-slate-400 mb-2"
                    )
                    ui.label("Your open positions will appear here").classes(
                        "text-slate-500"
                    )
                ...
                winning_positions = sum(
                    1 for p in positions_data if p.get("pnl", 0) > 0
                )

                # Summary Cards
                with ui.row().classes("w-full gap-4 flex-wrap mb-4"):
                    Components.kpi_card(
                        "Total Positions", total_positions, suffix=" open"
                    )
                    Components.kpi_card(
                        "Total P&L",
                        total_pnl,
                    ...
                    Components.kpi_card("Win Rate", win_rate, suffix="%")
                ...
                with Components.card():
                    with ui.row().classes("w-full items-center justify-between mb-4"):
                        ...
                        ui.button(icon="refresh", on_click=load_positions_data).props(
                            "flat round"
                        ...
                        with ui.element("tbody"):
                            for position in positions_data:
                                ...
                                with ui.element("tr").classes(
                                    ...
                                    row.on(
                                        "click", make_click_handler(position["symbol"])
                                    )

                                    with ui.element("td").classes("py-3 pl-2"):
                                        ui.label(position.get("symbol", "")).classes(
                                    ...
                                    with ui.element("td").classes("py-3 text-right"):
                                        ui.label(f"₹{pnl:.2f}").classes(
                                            f"font-bold {pnl_color}"
                                        )
                                    with ui.element("td").classes(
                                        "py-3 text-right pr-2"
                                    ):
                                        icon = (
                                            "trending_up"
                                            if pnl_percent >= 0
                                        ...
                                        with ui.row().classes(
                                            f"items-center gap-1 justify-end {pnl_color}"
                                        ):
                                            ui.icon(icon, size="xs")
                                            ui.label(
                                                f"{abs(pnl_percent):.2f}%"
                                            ).classes("font-bold")
    ...
    async def show_position_details(symbol):
        ...
        with ui.dialog() as details_dialog, ui.card().classes(
            ...
            if "error" in position_data:
                ...
                # Header
                with ui.row().classes(
                    "w-full items-center justify-between border-b border-slate-700 pb-4"
                ):
                    with ui.row().classes("items-center gap-3"):
                        ui.icon("trending_up", size="lg").classes("text-indigo-500")
                        with ui.column().classes("gap-1"):
                            ui.label(position_data.get("symbol", "")).classes(
                                "text-2xl font-bold text-white"
                            )
                            side = position_data.get("side", "").upper()
                            side_color = (
                                "text-green-400" if side == "LONG" else "text-red-400"
                            )
                            ui.label(f"{side} Position").classes(
                                f"text-sm {side_color}"
                            )
                ...
                with ui.grid(columns=2).classes("w-full gap-4 mt-4"):
                    ...
                    with ui.column().classes("gap-1"):
                        ...
                        ui.label(str(position_data.get("quantity", 0))).classes(
                    ...
                    with ui.column().classes("gap-1"):
                        ...
                        ui.label(position_data.get("entry_date", "")).classes(
                            "text-lg text-white"
                        )

                    # Entry Price
                    with ui.column().classes("gap-1"):
                        ui.label("Entry Price").classes(
                            "text-xs text-slate-400 uppercase"
                        )
                        ui.label(f"₹{position_data.get('entry_price', 0):.2f}").classes(
                            "text-xl font-bold text-white"
                        )
                    ...
                    with ui.column().classes("gap-1"):
                        ui.label("Profit/Loss").classes(
                            "text-xs text-slate-400 uppercase"
                        )
                        ui.label(f"₹{pnl:.2f}").classes(
                            f"text-2xl font-bold {pnl_color}"
                        )
                    ...
                    with ui.column().classes("gap-1"):
                        ...
                        with ui.row().classes("items-center gap-2"):
                            icon = (
                                "trending_up" if pnl_percent >= 0 else "trending_down"
                            )
                            ui.icon(icon, size="lg").classes(pnl_color)
                            ui.label(f"{pnl_percent:.2f}%").classes(
                                f"text-2xl font-bold {pnl_color}"
                            )
