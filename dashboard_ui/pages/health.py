...
def render_page(state):
    ...
    Components.section_header("System Health", "Monitor system status and health", "health_and_safety")
            ...
            if "error" in health_data:
                with Components.card():
                    with ui.row().classes("items-center gap-4"):
                        ...
                        with ui.column():
                            ui.label("Health Check Failed").classes("text-xl font-bold text-red-400")
                            ui.label(f"Error: {health_data['error']}").classes("text-slate-400")
            else:
                    ...
                    with ui.row().classes("w-full items-center justify-between"):
                        with ui.row().classes("items-center gap-4"):
                            ...
                            if status == "healthy":
                                ...
                                ui.icon("warning", size="3xl").classes("text-yellow-500")
                            
                            with ui.column():
                                ui.label("System Status").classes("text-lg font-bold text-white")
                                ui.label(status.upper()).classes(
                                    f"text-2xl font-bold {'text-green-400' if status == 'healthy' else 'text-yellow-400'}"
                                )
                        
                        ui.button(icon="refresh", on_click=load_health_data).props("flat round").classes(
                ...
                with ui.row().classes("w-full gap-4 flex-wrap"):
                    ...
                    with Components.card().classes("flex-1 min-w-[280px]"):
                        with ui.row().classes("items-center gap-3 mb-2"):
                            ...
                            ui.label("Version Information").classes("text-sm font-bold text-slate-300")
                        
                        with ui.column().classes("gap-2"):
                            ui.label(f"v{health_data.get('version', 'Unknown')}").classes(
                    ...
                    with Components.card().classes("flex-1 min-w-[280px]"):
                        with ui.row().classes("items-center gap-3 mb-2"):
                            ...
                            ui.label("Last Check").classes("text-sm font-bold text-slate-300")
                        ...
                        with ui.column().classes("gap-2"):
                            ...
                            ui.label(formatted_time).classes("text-lg font-mono text-white")
                ...
                with Components.card():
                    ...
                    with ui.grid(columns=2).classes("w-full gap-4"):
                        ...
                        with ui.column().classes("gap-1"):
                            ui.label("Health Status").classes("text-xs text-slate-400 uppercase tracking-wider")
                        ...
                        # Version
                        with ui.column().classes("gap-1"):
                            ui.label("API Version").classes("text-xs text-slate-400 uppercase tracking-wider")
