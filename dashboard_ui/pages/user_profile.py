...
def render_page(state):
    """User Profile Page"""
    Components.section_header("User Profile", "View your account information", "account_circle")
        ...
        with profile_container:
            if "error" in profile_data or not profile_data.get("authenticated", False):
                ...
                with Components.card().classes("items-center justify-center py-12"):
                    ...
                    ui.label("Not Authenticated").classes("text-2xl font-bold text-slate-400 mb-2")
                    ui.label("Please login to view your profile").classes("text-slate-500 mb-6")
                    ...
                    if "error" in profile_data:
                        ui.label(f"Error: {profile_data['error']}").classes("text-xs text-red-400 mt-4")
            else:
                    ...
                    with ui.row().classes("w-full items-center justify-between"):
                        with ui.row().classes("items-center gap-4"):
                            ...
                            with ui.column():
                                ...
                                ui.label(profile_data.get("email", "")).classes("text-slate-400")
                                
                                with ui.row().classes("items-center gap-2 mt-2"):
                                    ui.chip("AUTHENTICATED", icon="check_circle", color="green").props(
                                        "dense square outline"
                                    )
                        
                        ui.button(icon="refresh", on_click=load_profile_data).props("flat round").classes(
                ...
                with ui.row().classes("w-full gap-4 flex-wrap"):
                    ...
                    with Components.card().classes("flex-1 min-w-[280px]"):
                        with ui.row().classes("items-center gap-3 mb-2"):
                            ...
                            ui.label("User ID").classes("text-sm font-bold text-slate-300")
                    ...
                    with Components.card().classes("flex-1 min-w-[280px]"):
                        with ui.row().classes("items-center gap-3 mb-2"):
                            ...
                            ui.label("Broker").classes("text-sm font-bold text-slate-300")
                    ...
                    with Components.card().classes("flex-1 min-w-[280px]"):
                        with ui.row().classes("items-center gap-3 mb-2"):
                            ...
                            ui.label("Account Type").classes("text-sm font-bold text-slate-300")
                ...
                if exchanges:
                    with Components.card():
                        ui.label("Enabled Exchanges").classes("text-lg font-bold text-white mb-4")
                        
                        with ui.row().classes("gap-2 flex-wrap"):
                            for exchange in exchanges:
                                ui.chip(exchange, color="indigo").props("outline square")
                
                # Additional Information Card
                with Components.card():
                    ui.label("Profile Details").classes("text-lg font-bold text-white mb-4")
                    
                    with ui.grid(columns=2).classes("w-full gap-4"):
                        # Name
                        with ui.column().classes("gap-1"):
                            ui.label("Full Name").classes("text-xs text-slate-400 uppercase tracking-wider")
                        ...
                        # Email
                        with ui.column().classes("gap-1"):
                            ui.label("Email").classes("text-xs text-slate-400 uppercase tracking-wider")
                        ...
                        # User ID
                        with ui.column().classes("gap-1"):
                            ui.label("User ID").classes("text-xs text-slate-400 uppercase tracking-wider")
                            ui.label(profile_data.get("user_id", "N/A")).classes(
                        ...
                        # Broker
                        with ui.column().classes("gap-1"):
                            ui.label("Broker").classes("text-xs text-slate-400 uppercase tracking-wider")
                            ui.label(profile_data.get("broker", "N/A")).classes(
                                "text-sm font-medium text-white"
                            )
